...
*Status:* **DECIDED (2026-06-21)** — **Cap'n Proto.** Zero-copy for the large, lazily-navigated model; schema-first polyglot codegen + evolution; promise-pipelining RPC fits the navigation-heavy client port. First-class now that the core is Rust (DD-016) — the FlatBuffers lean was a JVM-era artifact, and Cap'n Proto's weak JVM binding no longer applies *on this side* (the Rust↔JVM engine-port wire is separate — DD-009). **Resolution (coherence pass, 2026-06-21):** Cap'n Proto is the **persistence + transport projection** of the model; the *in-core working model* is a native mutable Rust graph keyed on the stable IDs (DD-004). Zero-copy buffers are read-optimized, so the live edit/merge representation is native and serializes *to* Cap'n Proto — the buffer is never the live model. **RPC surface (2026-06-22):** the navigation-heavy client port is **served in-process** — the heads drive `scylla_port::Session` directly (the MCP head marshals JSON-RPC ↔ port; the non-MCP client calls `Session` straight), so no Cap'n Proto RPC wire exists today (`model.capnp` is data `struct`s only; no `interface`, no `capnp-rpc`). The **promise-pipelining RPC** projection — for a *remote/networked* head — is **deferred** to a future adapter; Cap'n Proto remains the model-artifact **persistence** format now. The schema-choice rationale (promise-pipelining fits a navigation-heavy port) still holds; it just isn't realized as a live wire yet. **Deferral VALIDATED (2026-06-22), `spike/rpc-shape`:** the port was projected onto a throwaway Cap'n Proto RPC `interface` (`Session`/`Function` capabilities) and driven over a real two-party RPC — `function(gcd).callers().view()` pipelines (one round-trip) and reproduces the in-process port exactly, with **no port API change**. The port is StableId-keyed and returns owned data (no borrows / fat graph objects / chatty patterns), so it is wire-shaped. So the deferral is de-risked — the production surface can wait for a real remote head (DD-028 WASM / DD-027 collab) without risking a body rewrite. **RPC surface NOW BEING BUILT (2026-06-23, authorized):** the deferral is lifted — `crates/scylla-rpc` is the production projection the spike seeded: `schema/scylla_rpc.capnp` (`interface Session { info / functions / function }`, `interface Function { view / callers / rename / retype / comment }`) + `capnp-rpc` server impls backed by `scylla_port::Session`, with `serve_connection` / `connect` transport helpers. Verified in-process over a two-party RPC: `function(gcd).callers().view()` pipelines, `info`/`functions` project, and a rename round-trips with a blank name rejected as a `capnp::Error` (DD-021). **Remote head SHIPPED (2026-06-23):** the network transport + a remote client head close the loop — `scylla-rpc-serve <artifact> [host:port]` listens on TCP and serves the model to clients NOT co-located with the core; `scylla-rpc-connect <host:port> <info|functions|view|callers>` is the remote head that drives the port over the wire, with `function(id).callers().view()` pipelining into one round-trip. Verified end-to-end cross-process over real TCP (`tests/remote.rs`: spawn the server binary, the client binary reproduces gcd's callers = main). So DD-002 is **fully realized** — persistence (model-artifact) AND the promise-pipelining RPC transport, both on Cap'n Proto, the fifth head. **Hardened (2026-06-23):** capability-based auth (DD-035) — the bootstrap is an `Authenticator`, and a client gets NO `Session` capability (no authority at all) until it `login`s with the right token (`SCYLLA_RPC_TOKEN`; unset = open, with a server warning); a wrong token is a `capnp::Error`, never a `Session`. Verified cross-process: an absent/wrong token is denied with no data leak; the remote head also reaches verb parity (navigate / annotate / **diff**, the full matcher running server-side). A **concurrent-connection cap** (`SCYLLA_RPC_MAX_CONN`, default 64) bounds spawned tasks so a connection flood can't exhaust the server (the surplus connection is accepted then dropped), and a **handshake timeout** (`SCYLLA_RPC_HANDSHAKE_SEC`, default 10) drops any connection that doesn't authenticate in time — so a silent slow-loris connection can't squat a capped slot forever (the cap would otherwise be trivially defeated). Both verified over TCP. **TLS (2026-06-23):** the wire is plaintext by default but encrypts with `SCYLLA_RPC_TLS_CERT`/`SCYLLA_RPC_TLS_KEY` on the server and `SCYLLA_RPC_TLS_CA` (+`SCYLLA_RPC_TLS_SNI`) on the client (tokio-rustls, `ring` provider — no C-toolchain-heavy aws-lc) — so the token + the model don't cross the network in the clear; the lib stayed transport-agnostic (the TLS stream slots into the same generic `serve_with_timeout`/`connect`). Verified end-to-end with a self-signed cert: TLS navigate/diff works, a plaintext client against a TLS server is rejected with no data leak. The port needed **zero changes** (as the spike predicted) — the one bet we couldn't take back held its shape under the wire it was chosen for.
...
*Status:* **DECIDED (2026-06-21) — *safe.*** **Omit from v1**; the schema (DD-002) stays additive so provenance/confidence can slot in later without a break. Don't bloat the model now. **REALIZED (2026-06-24) — first step, exactly as planned:** additive `Provenance { producer, confidence }` on `UserFact` (`model.capnp` `@4`/`@5`); legacy artifacts load unchanged, defaulting to a certain `user`/100 fact (back-compat tested by hand-building a pre-provenance fact); the port stamps analyst annotations `user`/100 and re-anchoring (`retarget`) preserves provenance; producers stamp their own via `UserFact::with_provenance`. Validated by the dynamic-analysis seam spike (PR #92) as the no-regret groundwork. **Per-*edge* provenance landed next (2026-06-24):** an additive, sparse `edge_provenance` sidecar on `Function` (`model.capnp` `@13`, `EdgeProvenance { target, provenance }`) marks dynamically-resolved call edges, leaving `callees` and the matcher untouched (back-compat: empty on every existing model). **Provenance now does work in the merge (2026-06-24):** confidence-aware `collaborate` (DD-027, PR #98, shipped in v0.6.0) reconciles a fact disagreement by `Provenance::confidence` — so provenance is no longer merely *recorded* but *acted on*. (Coverage-aware merging — weighting a dynamic producer's partial coverage — remains the next refinement.)
...
single-user first? *Status:* **DIRECTION SET (2026-06-21)** — the bespoke server *dissolves*: collaboration is **model-artifact sync** (git-for-RE — share/sync/diff/merge), a consequence of the narrow-waist (see "Why It's Shaped This Way"). v1 scope (2026-06-21) — *safe:* **single-user; collaboration is manual artifact export/sync** (git-for-RE by hand), no merge tooling yet. The mechanism (artifact sync) was already settled. **REALIZED (2026-06-24) — the merge tooling now exists:** `scylla-merge::collaborate` merges two analysts' artifacts, and since v0.6.0 (PR #98) it is **confidence-aware** — a fact disagreement is settled by `Provenance::confidence` (DD-007): a unique winner clearing a 5-point margin auto-resolves (counted in `CollabReport.resolved_by_confidence`), while a near-tie is surfaced as a `Conflict`, never guessed (the same `WRONG = 0` "unique winner clearing a margin" discipline the re-anchoring matcher holds). Still single-user with manual artifact sync by design — the bespoke server stays dissolved; what changed is that *merging* the synced artifacts is now automated rather than by-hand.
