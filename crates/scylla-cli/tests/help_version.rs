...
fn help_flag_exits_0_on_stdout() {
    for flag in ["--help", "-h", "help"] {
        ...
        assert!(stdout.contains("usage:"), "{flag} stdout should contain usage");
        assert!(stdout.contains("materialize"), "{flag} stdout should describe verbs");
        assert!(stdout.contains("-h, --help"), "{flag} stdout should mention --help");
        assert!(stdout.contains("-V, --version"), "{flag} stdout should mention --version");
...
fn subcommand_help_exits_0_on_stdout() {
    for verb in ["info", "diff", "materialize", "decompile", "functions", "view", "callers", "merge"] {
        let (code, stdout, stderr) = run(&[verb, "--help"]);
        assert_eq!(code, 0, "{verb} --help should exit 0");
        assert!(stdout.contains("usage:"), "{verb} --help stdout should contain usage");
        assert!(stderr.is_empty(), "{verb} --help stderr should be empty: {stderr}");
    }
}
...
fn missing_args_exits_2_on_stderr() {
    ...
    assert!(stdout.is_empty(), "no args stdout should be empty: {stdout}");
    assert!(stderr.contains("usage:"), "no args stderr should contain usage");
}
...
fn unrecognized_args_exits_2_on_stderr() {
    ...
    assert!(stdout.is_empty(), "unrecognized verb stdout should be empty: {stdout}");
    assert!(stderr.contains("usage:"), "unrecognized verb stderr should contain usage");
}
...
fn search_help_vs_search_query() {
    ...
    assert!(stdout.contains("usage:"), "search --help should print usage");
