...
fn main() {
    ...
    let Some(artifact_path) = args.next() else {
        eprintln!("usage: scylla-serve <artifact.scylla> [port]   (default port 8000)");
        std::process::exit(2);
    };
    if artifact_path == "-h" || artifact_path == "--help" {
        println!("usage: scylla-serve <artifact.scylla> [port]   (default port 8000)");
        return;
    }
    let port: u16 = args.next().and_then(|p| p.parse().ok()).unwrap_or(8000);

    let artifact: Vec<u8> = match std::fs::read(&artifact_path) {
        Ok(b) => b,
        Err(e) => {
            eprintln!("scylla-serve: cannot read {artifact_path}: {e}");
            std::process::exit(1);
        }
    };
    ...
    for stream in listener.incoming().flatten() {
        thread::spawn(move || {
            let _ = handle(stream, artifact);
        });
    }
}

fn handle(mut stream: TcpStream, artifact: &[u8]) -> std::io::Result<()> {
